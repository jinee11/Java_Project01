...
import chapter07.*; //!주의 : 하위패키지는 적용이 안된다. 챕터 07에 있는 클래스만 import
import chapter07.ex01.*;//ex01패키지에 있는 클래스만 import
	...
	public static void main(String[] args) {
		...
		// 클래스 외부 구성요소
			...
			//1. 패키지 : 클래스를 조직화해서 관리하는 폴더
				//한 패키지내에 동일한 이름의 클래스는 올 수 없다.(폴더 안에 중복 파일 이름은 있을 수 없다)
				//외부에서 클래스를 가지고 오는 경우 기존의 클래스 이름과 동일한 경우
			//2. 임포트(import) : 다른 패키지에 있는 클래스를 사용하려는 경우 : import
		
		//Student 클래스의 객체 생성 : 다른패키지에 존재하는 클래스
				//chapter08.x02.Student.java 
		
		//기본생성자 호출
		//Student = 클래스 밖에서 import
		Student s1 = new Student(); //리모컨, 객체 생성
		s1.print();
		System.out.println();
		...
		//Car 클래스는 import없이 전체이름(패키지이름.클래스이름)으로 사용
